      ...
      <q-card style="width: 800px; max-width: 80vw;">
        ...
        <q-card-section v-if="opened">
          ...
          <q-input
            ...
            <draggable v-model="data.enum" :item-key="(item) => data.enum.indexOf(item)">
              <template #item={element}>
                <div class="q-chip row no-wrap inline items-center q-chip-small bg-primary text-white draggable">
                  <div class="q-chip-main ellipsis draggable">{{element}}</div>
                  <div class="q-chip-side q-chip-close q-chip-right row flex-center" @click="deleteChoice(element)"><i aria-hidden="true" class="q-icon cursor-pointer material-icons">cancel</i></div>
                </div>
              </template>
            </draggable>
          <q-checkbox
          ...
          >
            <q-btn-dropdown size="md" label="Add validator">
              <q-list>
                <q-item clickable v-for="(v, id) in validatorsByType(data.type)" :key="id" v-close-popup v-on:click="addValidator(id)" :title="v.description">
                  <q-item-label>
...
export default {
  ...
  data () {
    return {
      ...
      validators: [], // @todo: restore this functionality // this.$store.getters.validatorDict, // t{unique: {id: 'unique', name: 'Unique'}, foo: {id: 'foo', name: 'Foo'}},
      add_validator: null,
      option: null
